...
Основная особенность GigaChain — умение работать с русскоязычной нейросетью [GigaChat](https://giga.chat/).
За это отвечает модуль [GigaChat](https://github.com/ai-forever/gigachat), который автоматически формирует запросы в соответствии с форматом [GigaChat API](https://developers.sber.ru/docs/ru/gigachat/api/overview) и передает их в выбранную [модель](https://developers.sber.ru/docs/ru/gigachat/models).

При этом библиотека полностью совместима со всеми компонентами LangChain.
