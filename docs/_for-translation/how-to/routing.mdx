# Маршрутизация кода с учетом входных данных
...
В этом разделе оба способа продемострированны на примере двухэтапной последовательности, где сначала с помощью вопроса определяется о чем задан вопрос, после чего вызывается ветвь кода с соответствующей цепочкой.
## Подготовка примера

Зададим цепочку, которая определяет к чему относится входящий вопрос: к `LangChain`, `Anthropic` или `Other`.
...
Создание трех соответствующих подцепочек.
...
## Создание собственной функции (рекомендуется)
...
# Маршрутизация на основе семантического сходства

Для вызова запросов с наиболее подходящими промптами полезно использовать эмбеддинги.
